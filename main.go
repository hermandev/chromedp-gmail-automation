...
func OpenBrowserV2(data []User) {
	var wg sync.WaitGroup

	// Channel untuk mengontrol proses menjalankan browser secara berurutan
	done := make(chan bool)
	...
	for _, user := range data {
		...
		go func(user User) {
			...
			// Buat context baru untuk setiap goroutine
			...
			// Jalankan tugas-tugas chromedp dalam goroutine
			if err := chromedp.Run(ctx, chromedp.Tasks{
				runTask(chromedp.Navigate("https://accounts.google.com/"), "Open Google Account"),
				runTask(chromedp.WaitVisible("#identifierId", chromedp.ByID), "Check Input Email"),
				runTask(chromedp.SendKeys("#identifierId", user.Email, chromedp.ByID), "Input Email"),
			...
			// Tambahkan delay setelah setiap iterasi
			time.Sleep(time.Minute)
			done <- true
		}(user)
	...
	// Tutup channel done untuk memberi tahu bahwa semua proses sudah selesai
	close(done)
}
